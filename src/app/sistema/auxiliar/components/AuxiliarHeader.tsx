...
import { 
  getNotificacionesAuxiliar, 
  marcarComoLeidaAuxiliar, 
  marcarTodasComoLeidasAuxiliar,
  getConteoNotificacionesNoLeidas 
} from '../notificaciones-actions';

interface AuxiliarHeaderProps {
    onMenuToggle?: () => void;
}
...
export function AuxiliarHeader({ onMenuToggle }: AuxiliarHeaderProps) {
  ...
  const marcarLeida = async (id: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    try {
      await marcarComoLeidaAuxiliar(id);
  ...
  const marcarTodasLeidas = async () => {
    try {
      await marcarTodasComoLeidasAuxiliar();
      await cargarNotificaciones();
  ...
  const handleLogout = () => {
    toast.message('¿Cerrar sesión?', {
      description: 'Se cerrará tu acceso al panel administrativo auxiliar.',
      action: {
        label: 'Cerrar Sesión',
              ...
              {showNotifMenu && (
                <div className="absolute right-0 mt-3 w-96 rounded-2xl bg-white shadow-2xl shadow-slate-200/50 border border-slate-100 animate-in fade-in zoom-in-95 duration-200 z-50">
                  ...
                  <div className="p-3 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
                    <Link 
                      href="/sistema/auxiliar/notificaciones"
                ...
                >
                    <div className="text-right hidden sm:block">
                        <p className="text-sm font-semibold text-slate-800 leading-none mb-1">
                            {session?.user?.name?.split(' ')[0] || 'Gestor'}
                        </p>
                ...
                {showUserMenu && (
                    <div className="absolute right-0 mt-3 w-60 rounded-2xl bg-white p-2 shadow-2xl shadow-slate-200/50 border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
                        <div className="px-3 py-2 leading-none mb-2">
                             <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Cuenta Institucional</p>
                             <p className="text-xs text-slate-500 mt-1 truncate">{session?.user?.email}</p>
                        </div>
                        <Link href="/sistema/auxiliar/perfil" className="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl transition-colors font-medium">
                            <User className="h-4 w-4" /> Perfil de Usuario
                        </Link>
                        <Link href="/sistema/auxiliar/configuracion" className="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl transition-colors font-medium">

...
import { getDashboardStatsAuxiliar } from '../actions';
...
export default function AuxiliarDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
        try {
            setLoading(true);
            const stats = await getDashboardStatsAuxiliar();
            setData(stats);
        } catch (error) {
  ...
  if (loading) {
    return (
        <div className="min-h-[60vh] flex flex-col items-center justify-center bg-[#fafaf9]">
            ...
            <p className="mt-4 text-[10px] font-semibold text-slate-400 uppercase tracking-[0.2em]">Sincronizando Entorno Administrativo...</p>
        </div>
    );
  }
    ...
    <div className="max-w-[1440px] mx-auto space-y-6 pb-20 px-4 md:px-6 lg:px-8 mt-4 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div className="space-y-1">
            <h1 className="text-xl font-semibold text-slate-900 tracking-tight">Panel Administrativo</h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            <button className="h-10 px-4 rounded-lg bg-[#8B1E1E] text-white flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider hover:bg-[#a32424] transition-all shadow-lg shadow-red-900/10">
                <Plus className="w-3.5 h-3.5" /> Nueva Acción
            </button>
        </div>
      </div>

      {/* Statistics Hub - Compact High-Performance Grid */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatWidget 
            ...
            value={data?.stats?.estudiantesActivos || 0} 
            icon={TrendingUp} 
            status="+2.4% vs mes anterior"
          ...
          <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden min-h-[400px]">
                  <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between bg-[#FAFAFA]/50">
                      ...
                      <Link href="/sistema/auxiliar/validacion" className="text-[10px] font-bold text-slate-400 hover:text-[#8B1E1E] transition-colors border border-slate-200 px-3 py-1 rounded-md">Ver Auditoría</Link>
                  </div>
                      ...
                      {data?.prioritarias?.length > 0 ? data.prioritarias.map((task: any, i: number) => (
                          <div key={i} className="px-8 py-5 flex items-center justify-between hover:bg-slate-50/50 transition-colors group">
                              ...
                              <div className="flex items-center gap-6 shrink-0">
                                  ...
                                  <Link 
                                    href={task.type === 'HORAS' ? '/sistema/auxiliar/validacion' : '/sistema/auxiliar/postulaciones'}
                                    className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-[#8B1E1E] group-hover:text-white transition-all transform group-hover:translate-x-1"
                                  >
                  ...
                  {data?.prioritarias?.length > 0 && (
                      <div className="p-4 text-center bg-slate-50/30 border-t border-slate-50">
                           <button className="text-[10px] font-bold text-slate-400 hover:text-[#8B1E1E] uppercase tracking-[0.2em] transition-colors">Cargar historial de hoy</button>
                      </div>
                  )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <AccessCard 
                    title="Convocatorias Institucionales" 
                    desc="Explora y supervisa los proyectos activos de servicio social."
                    icon={FileText}
                    href="/sistema/auxiliar/convocatorias"
                  />
                  <AccessCard 
                    title="Base Estudiantil" 
                    desc="Expedientes técnicos y seguimiento de horas acumuladas."
                    icon={UserCircle2}
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-8">
                  <div className="space-y-6">
                      <div className="flex items-center justify-between">
                          <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Información Crítica</h3>
                          <AlertCircle className="w-4 h-4 text-slate-200" />
                      </div>
                      <div className="space-y-6">
                           <InfoItem 
                                title="Cierre de Mes Administrativo" 
                                date="Viernes, 20 de Septiembre"
                                desc="Recuerda conciliar todas las planillas físicas antes de la fecha límite para reportes semestrales."
                           />
                           <div className="h-px bg-slate-50" />
                           <InfoItem 
                                title="Auditoría de Postulaciones" 
                                date="Pendiente"
                                desc="Hay 12 estudiantes con documentos en revisión que requieren acción inmediata del auxiliar."
                           />
                      </div>
                  </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-5">
                  <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-0.5">
                          <h4 className="text-sm font-semibold text-slate-800">Guía del Auxiliar</h4>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Protocolo Institucional</p>
                      </div>
                  </div>
                  <p className="text-xs text-slate-500 font-medium leading-relaxed">
                      Consulta los procedimientos estandarizados para validación de horas y gestión de proyectos de servicio social.
                  </p>
...
function AccessCard({ title, desc, icon: Icon, href }: any) {
    return (
        <Link href={href} className="flex flex-col p-6 bg-white rounded-2xl border border-slate-100 shadow-sm hover:border-[#8B1E1E]/20 hover:shadow-xl hover:shadow-slate-200/40 transition-all group min-h-[140px]">
            <div className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center mb-6 group-hover:bg-[#8B1E1E]/5 group-hover:text-[#8B1E1E] transition-colors">
                <Icon className="w-5 h-5" />
            </div>
            <div className="space-y-1">

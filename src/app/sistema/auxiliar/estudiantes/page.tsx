...
import { getEstudiantesDetalladosAuxiliar } from '../actions';
...
export default function AuxiliarEstudiantesPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                setLoading(true);
                const data = await getEstudiantesDetalladosAuxiliar();
                setEstudiantes(data);
            } catch (error) {
                toast.error('Error al cargar la base de datos estudiantil');
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="max-w-[1440px] mx-auto space-y-8 pb-20 px-4 md:px-0 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 py-2 border-b border-slate-50">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Base de Datos Centralizada</span>
                    </div>
                    <div className="space-y-1">
                        <h1 className="text-3xl font-semibold text-slate-900 tracking-tight">Directorio de Estudiantes</h1>
                        <p className="text-slate-500 text-sm font-medium leading-relaxed max-w-2xl">
                            Administra los expedientes técnicos y monitoriza el cumplimiento de horas de servicio social obligatorio.
                        </p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button className="h-10 px-5 bg-[#8B1E1E] text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-[#a32424] transition-all shadow-lg shadow-red-900/10 flex items-center gap-2">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filtered.map((student) => {
                        ...
                        return (
                            <div key={student.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:border-[#8B1E1E]/20 hover:shadow-xl hover:shadow-slate-200/50 transition-all group">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 font-bold border border-slate-100 group-hover:bg-[#8B1E1E] group-hover:text-white group-hover:border-[#8B1E1E] transition-all duration-300 shadow-sm text-xs">
                                ...
                                <div className="space-y-1 mb-6 min-h-[56px]">
                                    <h3 className="font-semibold text-slate-900 leading-tight group-hover:text-[#8B1E1E] transition-colors">{student.nombreCompleto}</h3>
                                ...
                                <div className="pt-4 border-t border-slate-50 flex gap-2">
                                    <Link 
                                        href={`/sistema/auxiliar/estudiantes/${student.id}`}
                                        className="flex-1 h-9 bg-slate-50 text-slate-600 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-[#8B1E1E] hover:text-white transition-all flex items-center justify-center gap-2 border border-slate-50"
                                    >
                                        <FileSearch className="w-3 h-3" /> Ficha Técnica
                                    </Link>
                                    <a 
            ...
            {!loading && filtered.length === 0 && (
                <div className="py-32 text-center bg-white rounded-3xl border border-slate-100 border-dashed">
                    ...
                    <h3 className="text-sm font-semibold text-slate-900 mb-1">Sin coincidencias en el directorio</h3>
                    <p className="text-xs text-slate-400 font-medium">No se encontraron estudiantes registrados con esos criterios.</p>
                </div>
            )}
        </div>
    );

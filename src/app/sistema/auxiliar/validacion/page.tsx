...
import { 
    getReportesParaValidar, 
    validarReporteAuxiliar, 
    registrarHorasEstudianteAuxiliar, 
    getEstudiantesActivos,
    getActividadesPorConvocatoria
} from '../actions';
...
export default function ValidacionHorasPage() {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const data = await getReportesParaValidar();
            setReportes(data);
        } catch (error) {
    ...
    const handleValidar = async (id: string, horas: number) => {
        try {
            await validarReporteAuxiliar(id, horas, 'Aprobación técnica Auxiliar');
        ...
        <div className="max-w-7xl mx-auto space-y-12 pb-20 px-4 md:px-0 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pt-4">
                ...
                <button 
                    ...
                    className="h-12 px-8 bg-[#8B1E1E] text-white rounded-xl text-sm font-semibold hover:shadow-xl hover:shadow-red-900/10 transition-all flex items-center gap-3 active:scale-95"
                >
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
                <div className="relative flex-1 w-full group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-[#8B1E1E] transition-colors" />
                    <input 
                        placeholder="Filtrar por nombre del estudiante o actividad..." 
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full h-11 bg-white border border-slate-100 rounded-xl pl-11 pr-4 text-xs font-medium focus:ring-4 focus:ring-[#8B1E1E]/5 focus:border-[#8B1E1E]/20 transition-all placeholder:text-slate-300"
                    />
                </div>
                                    ...
                                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            <button 
                                                ...
                                                className="h-10 px-6 rounded-xl bg-slate-50 border border-slate-100 text-[#8B1E1E] text-xs font-bold uppercase tracking-widest hover:bg-[#8B1E1E] hover:text-white hover:border-[#8B1E1E] transition-all flex items-center justify-center gap-2 ml-auto"
                                            >
...
function RegisterHoursModal({ onClose, onSuccess }: { onClose: () => void, onSuccess: () => void }) {
    ...
    useEffect(() => {
        Promise.all([getEstudiantesActivos(), getConvocatoriasAuxiliar()]).then(([ests, convs]) => {
            setEstudiantes(ests); setConvocatorias(convs);
        });
    }, []);

    useEffect(() => {
        if (selectedConv) getActividadesPorConvocatoria(selectedConv).then(setActividades);
        else setActividades([]);
    }, [selectedConv]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            await registrarHorasEstudianteAuxiliar(formData);
                ...
                <form className="space-y-8" onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                            ...
                            <select 
                                className="w-full h-12 bg-slate-50 border border-slate-100 rounded-xl px-4 text-xs font-semibold focus:ring-4 focus:ring-[#8B1E1E]/5 transition-all"
                        ...
                        <div className="space-y-2">
                            ...
                            <select 
                                className="w-full h-12 bg-slate-50 border border-slate-100 rounded-xl px-4 text-xs font-semibold focus:ring-4 focus:ring-[#8B1E1E]/5 transition-all disabled:opacity-20"
                        ...
                        <div className="space-y-2">
                            ...
                            <input 
                                ...
                                className="w-full h-12 bg-slate-50 border border-slate-100 rounded-xl px-4 text-sm font-semibold focus:ring-4 focus:ring-[#8B1E1E]/5 transition-all"
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea 
                            className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-medium focus:ring-4 focus:ring-[#8B1E1E]/5 min-h-[100px] transition-all"
                    ...
                    <button 
                        ...
                        className="w-full h-14 bg-[#8B1E1E] text-white rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-[#a32424] transition-all shadow-lg shadow-red-900/10 disabled:opacity-50 flex items-center justify-center gap-3"
                    >

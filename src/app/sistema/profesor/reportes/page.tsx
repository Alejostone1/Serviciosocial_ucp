        ...
        <div className="space-y-8 animate-in fade-in duration-500">
                                        ...
                                        <td className="px-8 py-7 text-right">
                                            <div className="flex items-center justify-end gap-3">
                                                <button 
                                                    ...
                                                    className="w-10 h-10 flex items-center justify-center rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 transition-all border border-blue-100 shadow-sm"
                                                    title="Ver detalles"
                                                >
            ...
            {mostrarDetalles && reporteSeleccionado && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-3xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col border border-white/20 animate-in zoom-in-95 duration-300">
                        {/* Modal Header */}
                        <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 bg-slate-50/50">
                            <div>
                                <h2 className="text-xl font-black text-slate-900 leading-none">Detalles del Reporte</h2>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">
                                    Ref: {reporteSeleccionado.id.slice(0, 8)}
                                </p>
                            </div>
                            <button 
                                onClick={() => setMostrarDetalles(false)}
                                className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-slate-200 transition-colors text-slate-400"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="overflow-y-auto p-8 space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3 text-slate-400">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest">Estudiante</span>
                                    </div>
                                    <div>
                                        <p className="font-black text-slate-900 text-lg leading-tight">
                                            {reporteSeleccionado.estudiante.primer_nombre} {reporteSeleccionado.estudiante.primer_apellido}
                                        </p>
                                        <p className="text-slate-500 font-medium text-xs mt-1">{reporteSeleccionado.estudiante.correo || 'Sin correo registrado'}</p>
                                        <p className="text-slate-400 font-bold text-[10px] uppercase mt-1">Doc: {reporteSeleccionado.estudiante.numero_documento}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3 text-slate-400">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest">Horas y Fecha</span>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="bg-[#8B1E1E]/5 px-4 py-2 rounded-2xl border border-[#8B1E1E]/10">
                                            <p className="text-2xl font-black text-[#8B1E1E] leading-none">{reporteSeleccionado.horas_reportadas}</p>
                                            <p className="text-[9px] font-black text-[#8B1E1E] uppercase tracking-tighter mt-1 text-center">Horas</p>
                                        </div>
                                        <div>
                                            <p className="font-bold text-slate-700">Realizado el:</p>
                                            <p className="text-slate-500 font-medium">
                                                {reporteSeleccionado.fecha_actividad 
                                                    ? new Date(reporteSeleccionado.fecha_actividad).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })
                                                    : 'No especificado'}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-3 text-slate-400">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest">Descripción del Trabajo</span>
                                </div>
                                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 text-slate-700 font-medium leading-relaxed">
                                    {reporteSeleccionado.descripcion_trabajo || 'No se proporcionó descripción.'}
                                </div>
                            </div>

                            {/* Student Notes */}
                            {reporteSeleccionado.notas_estudiante && (
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3 text-slate-400">
                                        <MessageSquare className="w-4 h-4" />
                                        <span className="text-[10px] font-black uppercase tracking-widest">Notas Adicionales</span>
                                    </div>
                                    <div className="bg-amber-50/50 rounded-2xl p-6 border border-amber-100 text-amber-900 font-medium italic">
                                        {reporteSeleccionado.notas_estudiante}
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-3 text-slate-400">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest">Archivos de Evidencia ({reporteSeleccionado.evidencias?.length || 0})</span>
                                </div>
                                
                                {reporteSeleccionado.evidencias && reporteSeleccionado.evidencias.length > 0 ? (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        {reporteSeleccionado.evidencias.map((archivo: any) => (
                                            <a 
                                                key={archivo.id} 
                                                href={archivo.archivo_url} 
                                                target="_blank" 
                                                rel="noopener noreferrer"
                                                className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 hover:bg-[#8B1E1E]/5 hover:border-[#8B1E1E]/10 transition-all group"
                                            >
                                                <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-white group-hover:text-[#8B1E1E] transition-all">
                                                    <Download className="w-5 h-5" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-xs font-bold text-slate-700 truncate group-hover:text-[#8B1E1E] transition-colors">
                                                        {archivo.archivo_nombre || 'Archivo sin nombre'}
                                                    </p>
                                    ...
                                    <div className="flex items-center gap-3 p-6 rounded-2xl border-2 border-dashed border-slate-100 text-slate-400 italic font-medium">
                                        <AlertCircle className="w-5 h-5 opacity-50" />
                                        No se adjuntaron evidencias para este reporte.
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="px-8 py-6 border-t border-slate-100 flex items-center justify-between bg-slate-50/30">
                            <button 
                                onClick={() => setMostrarDetalles(false)}
                                className="px-6 h-11 rounded-xl bg-white border border-slate-100 text-slate-500 font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all"
                            >
                                Cerrar
                            </button>
                            <div className="flex items-center gap-3">
                                <button 
                                    onClick={() => {
                                        ...
                                        handleRechazar(reporteSeleccionado.id);
                                    }}
                                    className="px-6 h-11 rounded-xl bg-slate-50 text-slate-400 font-black text-xs uppercase tracking-widest hover:bg-rose-50 hover:text-rose-600 transition-all"
                                >
                                    Rechazar
                                </button>
                                <button 
                                    onClick={() => {
                                        setMostrarDetalles(false);
                                        handleAprobar(reporteSeleccionado.id, reporteSeleccionado.horas_reportadas);
                                    }}
                                    className="px-8 h-12 rounded-xl bg-[#8B1E1E] text-white font-black text-xs uppercase tracking-widest shadow-lg shadow-[#8B1E1E]/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                                >
                                    Aprobar definitivamante
                                </button>
                            </div>
                        </div>
                    </div>

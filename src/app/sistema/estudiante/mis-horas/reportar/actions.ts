...
const reporteSchema = z.object({
    ...
    horas_reportadas: z.number().min(0.5, 'Mínimo 0.5 horas').max(12, 'Máximo 12 horas por día'),
    ...
    archivos_soporte: z.array(z.object({
        nombre: z.string(),
        url: z.string(),
        tipo: z.string(),
        tamaño: z.number(),
...
export async function crearReporteHoras(formData: FormData) {
    ...
    try {
        ...
        // Crear el reporte con sus evidencias
        const nuevoReporte = await db.execute(async (prisma) => {
            return await prisma.reporteHoras.create({
                data: {
                    ...
                    // Agregar las evidencias de forma anidada
                    evidencias: validatedData.archivos_soporte ? {
                        create: validatedData.archivos_soporte.map(archivo => ({

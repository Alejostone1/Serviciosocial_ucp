...
// GET: Obtener notificaciones o plantillas
    ...
    try {
        ...
        if (tipo === 'plantillas') {
            const [plantillas, total] = await Promise.all([
                ...
                prisma.plantillaNotificacion.count(),
            ]);
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (accion === 'editar_plantilla') {
            ...
            const validatedData = crearPlantillaSchema.parse(data);
...
// DELETE: Eliminar plantilla o notificación
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (tipo === 'plantilla') {
            await prisma.plantillaNotificacion.delete({ where: { id } });

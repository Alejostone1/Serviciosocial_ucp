...
export function CrearPlantillaModal({ isOpen, onClose, onSuccess }: CrearPlantillaModalProps) {
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [formData, setFormData] = useState({
        nombre: '',
        tipo: 'SISTEMA' as typeof tiposNotificacion[number],
        asunto_correo: '',
        mensaje_plantilla: '',
        mensaje_whatsapp: '',
        variables: [] as string[]
            ...
            <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="flex items-center justify-between p-6 border-b">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">Crea plantillas reutilizables para notificaciones</p>
                    </div>
                    <button
                        onClick={handleClose}
                        ...
                        <div>
                            ...
                            {formData.variables.length > 0 && (
                                <div className="mt-2 text-xs text-gray-600">
                                    Variables disponibles: {formData.variables.map(v => `{{${v}}}`).join(', ')}
                                </div>
                            )}
                        </div>

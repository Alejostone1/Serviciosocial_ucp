...
export async function getPlantillas() {
    ...
    return await prisma.plantillaNotificacion.findMany({
        orderBy: { creado_en: 'desc' },
    });
}
...
export async function toggleEstadoPlantilla(id: string, actualEstado: boolean) {
    const session = await getSession();
    if (!session?.user?.id || session.user.role !== 'ADMINISTRADOR') {
        throw new Error('No autorizado');
    }
    ...
    await prisma.plantillaNotificacion.update({
        where: { id },
        data: { esta_activa: !actualEstado },
    });

    revalidatePath('/administrador/notificaciones');
}

export async function getNotificacionesMasivas() {
    const session = await getSession();
    if (!session?.user?.id || session.user.role !== 'ADMINISTRADOR') {
        throw new Error('No autorizado');
    }

    return await prisma.notificacionMasiva.findMany({
        orderBy: { creado_en: 'desc' },
    });
}

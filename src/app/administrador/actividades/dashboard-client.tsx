...
export function ActividadesDashboardClient({ 
    ...
    const [filtros, setFiltros] = useState({
        ...
        rol_creador_convocatoria: [] as string[]
    });
    ...
    const aplicarFiltros = useCallback(async () => {
        ...
        // Filtro por rol del creador de la convocatoria
        if (filtros.rol_creador_convocatoria.length > 0) {
            filtered = filtered.filter(a => a.convocatoria?.publicador && filtros.rol_creador_convocatoria.includes(a.convocatoria.publicador.rol));
        }
            ...
            cell: (item) => (
                <div className="flex items-center justify-end gap-3">
                    <Link
                        ...
                        className="flex items-center justify-center w-12 h-12 rounded-full text-blue-600 bg-blue-50 border-2 border-blue-100 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm active:scale-95 group"
                    ...
                    <button
                        ...
                        className={`flex items-center justify-center w-12 h-12 rounded-full border-2 transition-all shadow-sm active:scale-95 group ${
                            item.esta_activa 
                                ? 'text-amber-500 bg-amber-50 border-amber-100 hover:bg-amber-500 hover:text-white hover:border-amber-500' 
                                : 'text-emerald-500 bg-emerald-50 border-emerald-100 hover:bg-emerald-500 hover:text-white hover:border-emerald-500'
                        }`}
                    ...
                    <button
                        ...
                        className="flex items-center justify-center w-12 h-12 rounded-full text-red-500 bg-red-50 border-2 border-red-100 hover:bg-red-600 hover:text-white hover:border-red-600 transition-all shadow-sm active:scale-95 group"
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="space-y-1.5">
                                    {[
                                        ...
                                        { id: 'ALIADO_ESTRATEGICO', label: 'Aliado Estratégico' }
                                    ].map(rol => (
                                        <label key={rol.id} className="flex items-center group cursor-pointer">
                                            <input
                                                type="checkbox"
                                                checked={filtros.rol_creador_convocatoria.includes(rol.id)}
                                                onChange={(e) => {
                                                    if (e.target.checked) {
                                                        setFiltros(prev => ({ ...prev, rol_creador_convocatoria: [...prev.rol_creador_convocatoria, rol.id] }));
                                                    } else {
                                                        setFiltros(prev => ({ ...prev, rol_creador_convocatoria: prev.rol_creador_convocatoria.filter(id => id !== rol.id) }));
                                                    }
                                                }}
                                                className="w-4 h-4 border-[#e2e8f0] rounded focus:ring-[#8B1E1E] text-[#8B1E1E]"
                                            />
                                ...
                                <div className="pt-4 border-t border-[#f1f5f9] mt-2">
                                    <button
                                        ...
                                        className="w-full py-2.5 text-[10px] font-bold text-white bg-[#8B1E1E] hover:bg-[#731919] rounded-xl transition-all shadow-md active:scale-95 uppercase tracking-widest"
                                    >
                                        Limpiar Filtros
                                    </button>
                                </div>
                ...
                {viewMode === 'list' ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        <AnimatePresence mode='popLayout'>
                            {filteredData.map((actividad, idx) => (
                                <motion.div
                                    layout
                                    key={actividad.id}
                                ...
                                >
                                    {/* Card Header con Tipo */}
                                    <div className="p-6 pb-4">
                                        <div className="flex justify-between items-start mb-4">
                                            <span className="text-[10px] font-extrabold uppercase tracking-widest px-3 py-1.5 rounded-xl bg-[#8B1E1E]/5 text-[#8B1E1E] border border-[#8B1E1E]/10 flex items-center gap-2">
                                                <Activity className="w-3 h-3" />
                                                {actividad.tipo_actividad.replace(/_/g, ' ')}
                                            </span>
                                            <StatusBadge status={actividad.esta_activa ? 'ACTIVA' : 'INACTIVA'} />
                                        </div>
                                        
                                        <h3 className="text-xl font-bold text-gray-900 leading-tight mb-2 group-hover:text-[#8B1E1E] transition-colors">
                                            {actividad.nombre}
                                        </h3>
                                        
                                        {actividad.convocatoria && (
                                            <div className="flex items-center gap-2 text-xs text-gray-500 font-medium">
                                                <BookOpen className="w-3.5 h-3.5 text-[#8B1E1E]/60" />
                                                <span className="truncate">{actividad.convocatoria.titulo}</span>
                                            </div>
                                        )}
                                    </div>

                                    {/* Metas Principales */}
                                    <div className="px-6 py-4 bg-gray-50/50 grid grid-cols-2 gap-4 border-y border-gray-50">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Horas Estimadas</p>
                                            <div className="flex items-center gap-2">
                                                <div className="p-1.5 bg-white rounded-lg border border-gray-100">
                                                    <Clock className="w-4 h-4 text-[#8B1E1E]" />
                                                </div>
                                                <span className="text-lg font-black text-gray-900">{actividad.horas_estimadas}h</span>
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Reportes</p>
                                            <div className="flex items-center gap-2">
                                                <div className="p-1.5 bg-white rounded-lg border border-gray-100">
                                                    <Users className="w-4 h-4 text-blue-600" />
                                                </div>
                                                <span className="text-lg font-black text-gray-900">{actividad.reportes.length}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-6 mt-auto">
                                        <div className="flex items-center justify-between gap-4">
                                            <div className="flex-1">
                                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Acciones de Gestión</p>
                                                <div className="flex items-center gap-3">
                                                    <Link
                                                        href={`/administrador/actividades/${actividad.id}`}
                                                        className="flex-1 h-14 rounded-2xl text-blue-600 bg-blue-50 border-2 border-blue-100 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all flex items-center justify-center shadow-sm active:scale-95 group/btn"
                                                        title="Ver detalles"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className={`flex-1 h-14 rounded-2xl transition-all border-2 flex items-center justify-center shadow-sm active:scale-95 group/btn ${
                                                    ...
                                                    >
                                                        <Power className="w-6 h-6 group-hover/btn:scale-110 transition-transform" />
                                                    </button>
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex-1 h-14 rounded-2xl text-red-500 bg-red-50 border-2 border-red-100 hover:bg-red-600 hover:text-white hover:border-red-600 transition-all flex items-center justify-center shadow-sm active:scale-95 group/btn"
                                                        title="Eliminar"
                                                    >

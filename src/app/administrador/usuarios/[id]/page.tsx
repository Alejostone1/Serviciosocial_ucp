...
export default async function UsuarioDetallePage({ params }: { params: { id: string } }) {
    const rawUsuario = await getUsuarioById(params.id);

    if (!rawUsuario) {
        notFound();
    }

    // Transformar objetos Decimal a números
    const usuario = transformDecimalsToNumbers(rawUsuario);
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {certificados.map((c: any) => (
                                        <Link 
                                            ...
                                            className="flex items-center justify-between p-4 border border-slate-200 rounded-md hover:border-[#8B1E1E] hover:bg-slate-50 transition-all group cursor-pointer"
                                        >
                                            <div className="space-y-1">
                                            ...
                                            <div className="p-2 border border-slate-200 rounded text-slate-400 group-hover:text-[#8B1E1E] group-hover:border-[#8B1E1E] transition-colors">
                                                <ExternalLink className="w-4 h-4" />
                                            </div>
                                        </Link>
                                    ))}

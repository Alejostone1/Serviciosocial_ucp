...
const schema = z.object({
    ...
    numero_documento: z.string().min(5, 'Mínimo 5 dígitos').max(20),
...
export function NuevoUsuarioModal({ isOpen, onClose, onSuccess, programas }: Props) {
    ...
    const onSubmit = async (values: FormData) => {
        ...
        try {
            ...
            toast.success('✅ Usuario creado', { id: toastId, description: `${user.correo} registrado como ${user.rol}. Se solicitará cambio de contraseña en el primer inicio.` });
            reset();
            onSuccess(user);
        } catch (e: any) {
            toast.error('Error al crear', { id: toastId, description: e.message ?? 'El correo o documento ya existe.' });
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input {...register('numero_documento')} className={inputClass} placeholder="Ej. 1094001234" />
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input {...register('correo')} type="email" className={inputClass} placeholder="usuario@ucp.edu.co" />
                        {errors.correo && <p className={errorClass}>{errors.correo.message}</p>}
                    </div>
                    <div>
                        <label className={labelClass}>Teléfono</label>
                        <input {...register('telefono')} className={inputClass} placeholder="Ej. 3001234567" />
                    </div>
                </div>

...
import { descargarPDFPublico } from '../actions';
...
export function ValidacionCertificadoClient({ resultado, codigo }: ValidacionCertificadoClientProps) {
    ...
    useEffect(() => {
        ...
        const generarQR = async () => {
            try {
                const urlValidacion = `${window.location.origin}/administrador/validar-certificado/${codigo}`;
    ...
    const descargarCertificado = async () => {
        try {
            ...
            const response = await descargarPDFPublico(codigo);
            ...
            if (response.success && response.pdfBase64) {
                ...
                a.href = url;
                a.download = response.filename || `Certificado-UCP-${codigo.slice(0, 8)}.pdf`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);
            } else {
                alert('No se pudo generar el certificado: ' + (response.error || 'Error desconocido'));
            }
        } catch (error) {
            console.error('Error al descargar:', error);
            alert('Error al procesar la descarga del certificado.');
        } finally {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="text-center text-sm text-gray-500">
                        ...
                        <p className="mt-1">
                            ...
                            <a
                                href={`/administrador/validar-certificado/${codigo}`}
                                className="text-blue-600 hover:underline font-medium"
                            >
                                /administrador/validar-certificado/{codigo}
                            </a>
                        </p>
                    </div>
                </div>

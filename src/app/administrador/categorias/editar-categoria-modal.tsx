...
export function EditarCategoriaModal({ isOpen, onClose, onSuccess, categoria }: Props) {
    ...
    useEffect(() => {
        if (categoria && isOpen) {
            reset({
                ...
                icono: categoria.icono || 'Tags',
                color_hex: categoria.color_hex || '#8B1E1E',
            });
        }
    ...
    const onSubmit = async (values: FormData) => {
        ...
        try {
            const updated = await editarCategoria(categoria.id, {
    ...
    return (
        <FormModal isOpen={isOpen} onClose={onClose} title="Editar Categoría" subtitle={`Modificando "${categoria.nombre}"`}>
